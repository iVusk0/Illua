//This is a part of firmware for NyamLedGame8x16 game console by Nanoslavic

//Drivers for AMTEGA328P 16MHz
...
uint8_t pReadKeys(void)
{
	...
	{
		pkeys_timer_read=millis(); pkeys_Value_Old=pkeys_Value;
		pkeys_Value=0;
		
		if (inbit(pkeys_k5_pin,pkeys_k5_bit)){pkeys_Value|=0x20;};//bit 5
		if (inbit(pkeys_k4_pin,pkeys_k4_bit)){pkeys_Value|=0x10;};//bit 4
		if (inbit(pkeys_k3_pin,pkeys_k3_bit)){pkeys_Value|=0x08;};//bit 3
		if (inbit(pkeys_k2_pin,pkeys_k2_bit)){pkeys_Value|=0x04;};//bit 2
		if (inbit(pkeys_k1_pin,pkeys_k1_bit)){pkeys_Value|=0x02;};//bit 1
		if (inbit(pkeys_k0_pin,pkeys_k0_bit)){pkeys_Value|=0x01;};//bit 0
		
		pkeys_Value|=pkeys_Unused; pkeys_Value_Change=0;
		if (pkeys_Value!=pkeys_Value_Old)
		{
			pkeys_Value_Change=1;
			pkeys_timer_cnt=millis();
		};
		return 1;
	};
...
{
	uint8_t new1,old;
	new1=pkeys_Value&NumBit;
	old=pkeys_Value_Old&NumBit;
...
{
	if ((pKeyPressed(NumBit)==2)|(pLongKeyPressed(NumBit)==2))
	{
		return 2;
	};
return 0;
